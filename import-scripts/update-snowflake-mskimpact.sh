...
source $IMPORT_SCRIPTS_DIR/dmp-import-vars-functions.sh
source $IMPORT_SCRIPTS_DIR/filter-clinical-arg-functions.sh

function report_error() {
    # Error message provided as an argument
    error_message="$1"
    echo -e "ERROR: $error_message. Exiting."
    exit 1
}

function pull_latest_data_from_git_repo() {
...
function setup_data_directories() {
    ...
    if [[ -d "$OUTPUT_TMPDIR" && "$OUTPUT_TMPDIR" != "/" ]] ; then
        rm -rf "$OUTPUT_TMPDIR"/*
        if [ $? -gt 0 ] ; then
    ...
    cp -a $MSK_SOLID_HEME_DATA_HOME/* $OUTPUT_TMPDIR
    if [ $? -gt 0 ] ; then
...
function generate_subset() {
    ...
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/generate-clinical-subset.py \
        --study-id="mskimpact" \
        --clinical-file="$OUTPUT_TMPDIR/data_clinical_patient.txt" \
        --filter-criteria="PARTA_CONSENTED_12_245=YES" \
        --subset-filename="$OUTPUT_TMPDIR/part_a_subset.txt"
    if [ $? -gt 0 ] ; then
    ...
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/merge.py \
        --study-id="mskimpact" \
        --subset="$OUTPUT_TMPDIR/part_a_subset.txt" \
    ...
    sh $IMPORT_SCRIPTS_DIR/subset-cdm-timeline-files.sh "mskimpact" $OUTPUT_DIR $OUTPUT_TMPDIR
    if [ $? -gt 0 ] ; then
...
function filter_clinical_cols() {
    ...
    PATIENT_INPUT_FILEPATH="$OUTPUT_DIR/data_clinical_patient.txt"
    ...
    SAMPLE_INPUT_FILEPATH="$OUTPUT_DIR/data_clinical_sample.txt"
...
function standardize_clinical_data() {
    PATIENT_INPUT_FILEPATH="$OUTPUT_DIR/data_clinical_patient.txt"
    PATIENT_OUTPUT_FILEPATH="$OUTPUT_DIR/data_clinical_patient.txt.standardized"
    SAMPLE_INPUT_FILEPATH="$OUTPUT_DIR/data_clinical_sample.txt"
    SAMPLE_OUTPUT_FILEPATH="$OUTPUT_DIR/data_clinical_sample.txt.standardized"

    # Standardize the clinical files to use NA for blank values
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/standardize_clinical_data.py -f "$PATIENT_INPUT_FILEPATH" > "$PATIENT_OUTPUT_FILEPATH" &&
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/standardize_clinical_data.py -f "$SAMPLE_INPUT_FILEPATH" > "$SAMPLE_OUTPUT_FILEPATH" &&
...
function add_seq_date_to_sample_file() {
    ...
    SAMPLE_INPUT_FILEPATH="$OUTPUT_DIR/data_clinical_sample.txt"
    SAMPLE_OUTPUT_FILEPATH="$OUTPUT_DIR/data_clinical_sample.txt.with_seq_date"
    KEY_COLUMNS="SAMPLE_ID PATIENT_ID"

    # Download seq_date.txt file from DataBricks
    $PYTHON3_BINARY $IMPORT_SCRIPTS_DIR/databricks_query_py3.py \
    ...
    $PYTHON3_BINARY $IMPORT_SCRIPTS_DIR/combine_files_py3.py -i "$SAMPLE_INPUT_FILEPATH" "$SEQ_DATE_FILEPATH" -o "$SAMPLE_OUTPUT_FILEPATH" -c $KEY_COLUMNS -m left &&
    mv "$SAMPLE_OUTPUT_FILEPATH" "$SAMPLE_INPUT_FILEPATH"
    if [ $? -gt 0 ] ; then
...
function add_metadata_headers() {
    ...
    INPUT_FILENAMES="$OUTPUT_DIR/data_clinical_sample.txt $OUTPUT_DIR/data_clinical_patient.txt"
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/add_clinical_attribute_metadata_headers.py -f $INPUT_FILENAMES -c "$CDD_URL" -s mskimpact -i $CDM_METADATA
}

function standardize_cna_data() {
    # Standardize the CNA file to use NA for blank values
    DATA_CNA_INPUT_FILEPATH="$OUTPUT_DIR/data_CNA.txt"
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/standardize_cna_data.py -f "$DATA_CNA_INPUT_FILEPATH"
}
...
function transpose_cna_data() {
    ...
    DATA_CNA_INPUT_FILEPATH="$OUTPUT_DIR/data_CNA.txt"
    $PYTHON3_BINARY $IMPORT_SCRIPTS_DIR/transpose_cna_py3.py "$DATA_CNA_INPUT_FILEPATH"
}

function standardize_mutations_data() {
    MUTATIONS_EXTD_INPUT_FILEPATH="$OUTPUT_DIR/data_mutations_extended.txt"
    NSOUT_MUTATIONS_INPUT_FILEPATH="$OUTPUT_DIR/data_mutations_non_signedout.txt"

    # Standardize the mutations files to check for valid values in the 'NCBI_Build' column
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/standardize_mutations_data.py -f "$MUTATIONS_EXTD_INPUT_FILEPATH" &&
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/standardize_mutations_data.py -f "$NSOUT_MUTATIONS_INPUT_FILEPATH"
}
...
function remove_duplicate_maf_variants() {
    ...
    MUTATIONS_EXTD_OUTPUT_FILEPATH="$OUTPUT_DIR/data_mutations_extended_merged.txt"
    NSOUT_MUTATIONS_OUTPUT_FILEPATH="$OUTPUT_DIR/data_mutations_non_signedout_merged.txt"

    # Remove duplicate variants from MAF files
    # CVR data can contain duplicates for a gene and its alias
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/remove-duplicate-maf-variants.py -i "$MUTATIONS_EXTD_INPUT_FILEPATH" -o "$MUTATIONS_EXTD_OUTPUT_FILEPATH" &&
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/remove-duplicate-maf-variants.py -i "$NSOUT_MUTATIONS_INPUT_FILEPATH" -o "$NSOUT_MUTATIONS_OUTPUT_FILEPATH" &&
...
function filter_germline_events_from_maf() {
    MUTATION_FILEPATH="$OUTPUT_DIR/data_mutations_extended.txt"
    MUTATION_FILTERED_FILEPATH="$OUTPUT_DIR/data_mutations_extended.txt.filtered"
    $PYTHON3_BINARY $IMPORT_SCRIPTS_DIR/filter_non_somatic_events_py3.py $MUTATION_FILEPATH $MUTATION_FILTERED_FILEPATH --event-type mutation
    if [ $? -gt 0 ] ; then
    ...
    MUTATION_FILEPATH="$OUTPUT_DIR/data_mutations_non_signedout.txt"
    MUTATION_FILTERED_FILEPATH="$OUTPUT_DIR/data_mutations_non_signedout.txt.filtered"
    $PYTHON3_BINARY $IMPORT_SCRIPTS_DIR/filter_non_somatic_events_py3.py $MUTATION_FILEPATH $MUTATION_FILTERED_FILEPATH --event-type mutation
    if [ $? -gt 0 ] ; then
...
# TODO We do this for sophia but not for AZ - what about snowflake?
function remove_sequenced_samples_header() {
    MAF_INPUT_FILEPATH="$OUTPUT_DIR/data_mutations_extended.txt"
    MAF_OUTPUT_FILEPATH="$OUTPUT_DIR/data_mutations_extended.txt.tmp"
...
function filter_replicated_maf_columns() {
    MUTATIONS_EXTENDED_INPUT_FILEPATH="$OUTPUT_DIR/data_mutations_extended.txt"
    MUTATIONS_EXTENDED_OUTPUT_FILEPATH="$OUTPUT_DIR/data_mutations_extended.txt.filtered"
    MUTATIONS_NONSIGNEDOUT_INPUT_FILEPATH="$OUTPUT_DIR/data_mutations_non_signedout.txt"
    MUTATIONS_NONSIGNEDOUT_OUTPUT_FILEPATH="$OUTPUT_DIR/data_mutations_non_signedout.txt.filtered"

    # Filter out the columns we want to exclude in both files
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/filter_clinical_data.py -c "$MUTATIONS_EXTENDED_INPUT_FILEPATH" -e "$MUTATIONS_EXTENDED_COLS_TO_FILTER" > "$MUTATIONS_EXTENDED_OUTPUT_FILEPATH" &&
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/filter_clinical_data.py -c "$MUTATIONS_NONSIGNEDOUT_INPUT_FILEPATH" -e "$NONSIGNEDOUT_COLS_TO_FILTER" > "$MUTATIONS_NONSIGNEDOUT_OUTPUT_FILEPATH" &&
...
function standardize_structural_variant_data() {
    DATA_SV_INPUT_FILEPATH="$OUTPUT_DIR/data_sv.txt"
    $PYTHON_BINARY $IMPORT_SCRIPTS_DIR/standardize_structural_variant_data.py -f "$DATA_SV_INPUT_FILEPATH"
}

function filter_germline_events_from_sv() {
    SV_FILEPATH="$OUTPUT_DIR/data_sv.txt"
    SV_FILTERED_FILEPATH="$OUTPUT_DIR/data_sv.txt.filtered"
    $PYTHON3_BINARY $IMPORT_SCRIPTS_DIR/filter_non_somatic_events_py3.py $SV_FILEPATH $SV_FILTERED_FILEPATH --event-type structural_variant
    if [ $? -gt 0 ] ; then
...
function push_updates_to_git_repo() {
    (   # Executed in a subshell to avoid changing the actual working directory
        ...
        cd $OUTPUT_DIR &&
        $GIT_BINARY add * &&
        $GIT_BINARY commit -m "chord data $CURRENT_DATE" &&
...
function zip_files_for_delivery() {
    ...
    zip "$OUTPUT_DIR/chord-data-$CURRENT_DATE.zip" $OUTPUT_DIR/*data*
}
...
function cleanup_repo() {
    ...
    bash $IMPORT_SCRIPTS_DIR/datasource-repo-cleanup.sh $OUTPUT_DIR
}

printTimeStampedDataProcessingStepMessage "Subset MSK-IMPACT"

# Pull latest from data repo
#if ! pull_latest_data_from_git_repo ; then
...
printTimeStampedDataProcessingStepMessage "Post-process clinical files"
...
printTimeStampedDataProcessingStepMessage "Post-process CNA file"
...
printTimeStampedDataProcessingStepMessage "Post-process MAF files"
...
printTimeStampedDataProcessingStepMessage "Post-process SV file"
...
printTimeStampedDataProcessingStepMessage "Finalize and validate study contents"
...
if ! push_updates_to_git_repo ; then
